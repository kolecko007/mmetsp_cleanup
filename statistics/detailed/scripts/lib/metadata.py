...
class MetadataCollection():
    ...
    def load(file_path):
        ...
        organisms = [Metadata(k, v) for k, v in org_hash.items()]
...
# Compares organism genus by metadata information
class Metadata():
    ...
    def date(self):
        return self.fetch_value('miscellaneous parameter', 'date_of_experiment')
    ...
    def prey_genus(self):
        ...
        filtered = [e for e in splitted if
                    len([w for w in stop_words if w in e]) == 0 and not e.capitalize().startswith('Ccmp')]
